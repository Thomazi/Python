...
class TestInvertida(unittest.TestCase):
    ...
    def test_invertida_2(self):
        # Cria a instância da classe imagem 
        im = pset1.Imagem(4, 1, [29, 89, 136, 200])
        # Chama o método 'invertida()' para receber o resultado
        resultado = im.invertida()

        # Verifica se cada pixel do resultado é igual a diferença de 255 e o pixel que é original
        for x in range(resultado.largura):
            for y in range(resultado.altura):
                pixel_original = im.get_pixel(x,y)
                pixel_invertido = resultado.get_pixel(x,y)
        # Por ultimo, vai verificar se o pixel invertido é igual 255 subtraído do valor do pixel original                                                              
        self.assertTrue(pixel_invertido, 255 - pixel_original)
